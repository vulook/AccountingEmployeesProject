...
- Клас **Period** містить дані періодів роботи працівника в його кар'єрі, що включає дати початку та закінчення роботи, а також зв'язки з відділом, посадою та працівником.
- Анотація **@ManyToOn** та **@JoinColumn** використовується для встановлення зв'язків з відділом (departmentId), посадою (positionId) та працівником (employeeId).
...
#### 3. service:
- Пакет service відповідає за бізнес-логіку та забезпечує виконання операцій з даними в рамках проєкту. Містить інтерфейси та реалізації сервісів, що надають функціональність для керування працівниками, періодами роботи, відділеннями та посадами.
...
- *GET /employees/department/{name}*: Отримання списку працівників за назвою відділу.
...
- *POST /departments/add*: Створення нового відділу.
- *DELETE /departments/delete/{id}*: Видалення відділу за ідентифікатором.
- *GET /departments/all*: Отримання списку всіх відділів.
...
- **departments.html**: Відображає та взаємодіє зі списком відділень, містить дві вкладки: *"All Departments"* та *"Add Department"*.
